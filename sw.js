// PROJECT A - Service Worker para notificações persistentes
// Este worker executa mesmo com o app fechado ou tela bloqueada
...
self.addEventListener('message', (event) => {
  if (event.data && event.data.type === 'SCHEDULE_ALERTS') {
    const { alerts, volume } = event.data;
    ...
    for (const alert of alerts) {
      ...
      if (delay > 0 && delay < 30 * 24 * 60 * 60 * 1000) { // até 30 dias
